...
class Vehicle:
    ...
    def send_message(self, destination):
        ...
        if reliability < 0.8:  # 80% success rate for message delivery
            print(f"Vehicle {self.id} sending message to Vehicle {destination.id}. Speed: {self.speed} units/s, Latency: {latency:.2f} seconds.")
            return True  # Message sent successfully
        else:
            print(f"Vehicle {self.id} failed to send message to Vehicle {destination.id}.")
...
class GreedyPerimeterStatelessRouting:
    ...
    def route(self, sender, destination, visited=None):
        ...
        if sender.send_message(closest_neighbor):
            ...
            self.total_messages += 1  # Increment for the final message routing
